...
import { updateDiscussion, getDiscussionById } from '@/store/Slices/discussionSlice'
...
function DiscussionUpdate() {
    ...
    const handleUpdateDiscussion = async (data) => {
        ...
        try {
            await dispatch(updateDiscussion({ discussionId, data: payload }));
            navigate('/discuss');
        } catch (err) {
            console.error(err);
        }
        ...
        <>
            ...
            <div className="w-full min-h-[calc(100vh-50px)] flex flex-col items-center py-10 bg-black">
                <div className="w-full max-w-7xl bg-neutral-900 rounded-lg relative">
                    ...
                    <form onSubmit={handleSubmit(handleUpdateDiscussion)} className="pt-12">
